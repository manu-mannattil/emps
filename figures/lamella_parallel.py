...
def params(p, gamma, direction="parallel"):
    ...
    if direction == "parallel":
        ...
        M_unif = M * (p ** 2 + phi_c**(2/3)) / (1 + phi_c**(2/3))
    else:
        h_eff = h*p
        gamma_eff = gamma * p**2 * (p**2 + phi_c**(2/3)) / (1 + phi_c**(2/3))
        M_unif = M * (1/p + phi_c**(2/3)) / (1 + phi_c**(2/3))

    T = T_c - kappa/a/h_eff**2 * (1 + np.log(gamma_eff))

    return h_eff, M_unif, gamma_eff, T

h_eff, M_unif, gamma_eff, T = params(p, gamma, direction="parallel")
...
def f_unif(x, adT):
    return 0.5 * (adT+M_unif) * x**2 + 0.25 * b * x**4
...
def stripe_unif(adT):
    ...
    mu = minimize(fun, bounds=(0, 0.5)).x
    x, y = fun(mu, False)
...
with plt.rc_context(rc):
    ...
    ax.set_ylim(-2.6, -2)
    ax.set_xlim(0.2 - 0.62, 0.2 + 0.62)

    ax.scatter([0.2], [-2.50], s=15, facecolor="k")
    ax.text(0.14, -2.515, "e", horizontalalignment="left")

    ax.scatter([0.57], [-2.50], s=15, facecolor="k")
    ax.text(0.51, -2.515, "f", horizontalalignment="left")

    ax.text(0.2, -2.35, r"L$_{\|}$", horizontalalignment="center")
    ax.text(0.43, -2.35, r"L$_{\|}$ + U", horizontalalignment="center")
    ax.text(-0.03, -2.35, r"U + L$_{\|}$", horizontalalignment="center")
    ax.text(0.6, -2.2, "U", horizontalalignment="center")
    ax.text(-0.2, -2.2, "U", horizontalalignment="center")
